...
Invoke-NativeGate '== Phase 8.3: format ==' { dart format lib test }
Invoke-NativeGate '== Phase 8.3: analyze ==' { flutter analyze }

Write-Host '== Phase 8.3: continuous zoom regressions ==' -ForegroundColor Cyan
...
Write-Host '== Phase 8.3: camera/photo/overlay/location/video guards ==' -ForegroundColor Cyan
...
Invoke-NativeGate '== Phase 8.3: full tests ==' { flutter test }
Invoke-NativeGate '== Phase 8.3: Android debug build ==' { flutter build apk --debug }

Write-Host 'Phase 8.3 automated gate PASS. On device verify slow pinch, fast pinch, direction reversal, second-finger join, 1x->max->1x, tap reset, then immediate photo capture.' -ForegroundColor Green
